...
int main() {
	...
	output("Original price: ", price);
	increase(&price);  // Pass the price by reference to modify it
	output("Increased price: ", price);
...
void increase(float *price) {
   if (*price <= 500) {
       *price *= 1.03;  // Increase by 3%
   } else if (*price <= 1000) {
       *price *= 1.05;  // Increase by 5%
   } else if (*price <= 3000) {
       *price *= 1.06;  // Increase by 6%
   } else {
       *price *= 1.07;  // Increase by 7%
   }
}
